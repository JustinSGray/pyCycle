...
if __name__ == "__main__":
    # Quick test
    print("Testing Tabular ThermoAdd functional interface")
    print("=" * 50)
    ...
    print(f"  Inflow: W={W} kg/s, h={h} J/kg, FAR={composition[0]}")
    print(f"  Fuel ratio: {ratio}")
    print(f"  Output: Wout={result.Wout:.4f} kg/s, mass_avg_h={result.mass_avg_h:.2f} J/kg")
    print(f"  Fuel W: {result.W_mix[0]:.4f} kg/s")
    print(f"  Composition out (FAR): {result.composition_out}")

    # Test flow mode
    print("\n2. Flow mode (stream mixing):")
    mixer2 = ThermoAdd(mix_mode='flow', mix_names='bleed')

    W_main = 28.0  # kg/s
    h_main = 422000.0  # J/kg
    comp_main = np.array([0.01])  # FAR = 0.01

    W_bleed = 2.0  # kg/s
    h_bleed = 300000.0  # J/kg
    comp_bleed = np.array([0.0])  # Pure air

    result2 = mixer2.compute(W=W_main, h=h_main, composition=comp_main,
                            W_mix={'bleed': W_bleed}, h_mix={'bleed': h_bleed},
                            composition_mix={'bleed': comp_bleed})

    print(f"  Main: W={W_main} kg/s, h={h_main} J/kg, FAR={comp_main[0]}")
    print(f"  Bleed: W={W_bleed} kg/s, h={h_bleed} J/kg, FAR={comp_bleed[0]}")
    print(f"  Output: Wout={result2.Wout:.4f} kg/s, mass_avg_h={result2.mass_avg_h:.2f} J/kg")
    print(f"  Composition out (FAR): {result2.composition_out}")

    # Verify mass conservation
    print("\n3. Mass conservation check:")
    expected_W = W_main + W_bleed
    print(f"  Expected Wout: {expected_W} kg/s")
    print(f"  Actual Wout: {result2.Wout} kg/s")
    print(f"  Match: {np.isclose(expected_W, result2.Wout)}")
